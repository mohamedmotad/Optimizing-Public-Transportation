...
ksql_table_topic = "TURNSTILE_SUMMARY"
arrival_topic = "org.chicago.cta.station.arrivals"
...
class Line:
    ...
    def process_message(self, message):
        ...
        if message.topic() == faust_table_topic: # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif message.topic() == arrival_topic: # Set the conditional to the arrival topic
            self._handle_arrival(message)
        elif message.topic() == ksql_table_topic:  # Set the conditional to the KSQL Turnstile Summary Topic
